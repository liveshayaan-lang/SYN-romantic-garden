...
const playHelpBaby = () => {
    try {
        const msg = new SpeechSynthesisUtterance("Help baby! Help baby!");
        msg.pitch = 2; // Highest pitch for thin voice
        msg.rate = 1.3;
        
        // Sometimes getVoices is empty globally, get it directly just in case
        let availableVoices = window.speechSynthesis.getVoices();
        if (availableVoices.length === 0) {
            availableVoices = synthVoices;
        }
        
        // Ensure we find a female voice
        let femaleVoice = availableVoices.find(v => 
            v.name.toLowerCase().includes('zira') || // Windows female
            v.name.toLowerCase().includes('hazel') || // UK female
            v.name.toLowerCase().includes('susan') || // female
            v.name.toLowerCase().includes('female') || 
            v.name.toLowerCase().includes('samantha') || // Mac female
            v.name.toLowerCase().includes('victoria') ||
            v.name.toLowerCase().includes('google us english') // Chrome default female
        );
        
        // If still no specific female voice found by name, try to avoid "David", "Mark", or "Male"
        if (!femaleVoice) {
            const possibleFemaleVoices = availableVoices.filter(v => 
            ...
            if (possibleFemaleVoices.length > 0) {
                // Pick the first English voice that is NOT David/Mark (usually Zira or similar)
                femaleVoice = possibleFemaleVoices[0];
            } else if (availableVoices.length > 1) {
                // Blind guess: pick the second voice (index 1), as index 0 is often default male
                femaleVoice = availableVoices[1];
            }
        }
...
export function StickmanBattle() {
  ...
  useEffect(() => {
    ...
    class Stickman {
      ...
      draw(ctx: CanvasRenderingContext2D, currentFrame: number) {
        ...
        ctx.strokeStyle = this.type === 'enemy' ? '#ff4d4d' : (this.type === 'you' ? '#ffb3c6' : '#a0c4ff');
        ctx.lineWidth = 2;
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
          ...
          if (this.type === 'me' || this.type === 'enemy') {
            ...
            if (this.isCarrying) {
                ...
                if (this.type === 'me') {
                    ...
                    ctx.fillStyle = '#00FFFF'; ctx.fillRect(8, -4, 24, 8); // Blade
            ...
            ctx.strokeStyle = this.type === 'me' ? '#a0c4ff' : '#ff4d4d';
            ctx.lineWidth = 2;
            ctx.stroke();
          } else {
            ctx.moveTo(0, -15);
        ...
        if (this.type === 'me' && currentFrame >= 650 && currentFrame < 700) {
            ...
            for (let j = 0; j < 3; j++) {
                ...
                const color = Math.random() > 0.5 ? '255, 50, 50' : '50, 150, 255';
            ...
            for(let i = 0; i < 6; i++) {
                ...
                ctx.strokeStyle = Math.random() > 0.5 ? '#ff3333' : '#33aaff';
        ...
        if (this.type === 'me' && this.downTimer > 0) {
          ...
          ctx.fillStyle = this.type === 'you' ? '#ff6b81' : '#70a1ff';
          ...
          if (this.type === 'you' && currentFrame > 500 && currentFrame < 700) {
             ...
             ctx.fillText('Help baby!', drawX + shakeX, drawY - 65 + shakeY);
          }
        }
      }
    }
